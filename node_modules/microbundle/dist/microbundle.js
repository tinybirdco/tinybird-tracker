  ...
  return {
    ...
    visitor: {
      Program(path, state) {
        ...
        const useHelper = state.opts.helper === true;
...
const createConfigItems = (type, items) => {
  return items.map(item => {
    ...
    return core.createConfigItem(value, {
      type
    });
  });
};
...
const getOutput = function ({
  ...
  try {
    ...
    const _temp9 = !main.match(/\.[a-z]+$/);

    return Promise.resolve(_temp9 ? _temp10(_temp9) : Promise.resolve(isDir(main)).then(_temp10));
  } catch (e) {
    return Promise.reject(e);
  }
};
